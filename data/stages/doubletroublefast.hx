...
{
	if (ProgressionUtil.songIsClear('double-trouble-(moe\'s-remix)') && (ClientPrefs.bfSkin != 'default' || ClientPrefs.gfSkin != 'default')) return;
...
function intro():Void
{
	...
	new FlxTimer().start(11, function(_) {
		triggerEventNote('camTween', '1100,1000,0.62', '6,smootherstepout'); // this is all temp bullshit until i set up the old stage
	});
